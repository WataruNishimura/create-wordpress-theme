...
cli
  .command(
    '[outDir]',
    'Generate wordpress theme developping envionment to given directory.'
  )
  .option(
    '--answers <json>',
  ...
  .action((outDir = '.', options) => {
    ...
    if (files.length) {
      return console.log(
        chalk.red(
          `Can't create ${outDir} because there's already a non-empty directory ${outDir} existing in path.`
        )
      );
    }
    console.log(chalk`✨  Generating Wordpress theme in {cyan ${outDir}}`);
    //sao settings
    const { answers, verbose } = options;
    sao({ generator, outDir, options, answers, verbose })
      .run()
      .catch((err) => {
        console.trace(err);
        process.exit(1);
